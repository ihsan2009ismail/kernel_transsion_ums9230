...
//sprd fault_type proc
static ssize_t show_fault_keyword(struct device *dev, struct device_attribute *attr, char * const buf)
{
	struct kbase_device *kbdev;
	ssize_t ret = 0;
	int i;
...
static ssize_t set_fault_keyword(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = 0;
	return ret;
}

static DEVICE_ATTR(fault_keyword, S_IRUGO | S_IWUSR, show_fault_keyword, set_fault_keyword);
//sprd fault_type proc
#endif
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	ret = scnprintf(buf, PAGE_SIZE, "%s %d cores r%dp%d 0x%04X\n", product_name,
